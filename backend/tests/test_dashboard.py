...
async def seed_priced_product(async_client: AsyncClient, *, price: str = "2.50") -> tuple[int, int]:
    """Creates a store/branch/product/store_product; returns (product_id, store_product_id)."""
    ...
    async with session_factory() as session:
        ...
        return product.id, store_product.id
...
async def test_checking_an_item_snapshots_cheapest_price_and_feeds_the_dashboard(async_client: AsyncClient) -> None:
    token = await get_access_token(async_client)
    product_id, _ = await seed_priced_product(async_client, price="2.50")
...
async def test_price_at_check_uses_cheapest_store_when_multiple_list_the_product(async_client: AsyncClient) -> None:
    token = await get_access_token(async_client)
    product_id, _ = await seed_priced_product(async_client, price="3.00")
    ...
    assert checked_item["price_at_check"] == "1.75"


async def test_dashboard_separates_current_and_previous_month_spend(async_client: AsyncClient) -> None:
    token = await get_access_token(async_client)
    product_id, _ = await seed_priced_product(async_client, price="5.00")
    shopping_list_id = await create_list(async_client, token)
...
async def test_dashboard_computes_savings_against_recent_price_history(async_client: AsyncClient) -> None:
    token = await get_access_token(async_client)
    product_id, store_product_id = await seed_priced_product(async_client, price="4.00")
...
async def test_budget_endpoint_and_dashboard_remaining_budget(async_client: AsyncClient) -> None:
    token = await get_access_token(async_client)
    product_id, _ = await seed_priced_product(async_client, price="20.00")
    shopping_list_id = await create_list(async_client, token)
    await add_and_check_item(async_client, token, shopping_list_id, product_id, quantity=1)
